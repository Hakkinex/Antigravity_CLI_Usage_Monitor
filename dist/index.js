    ...
    switch (command) {
        ...
        case 'quota':
            await quotaCommand({
                ...
                method: (readValue(args, '--method', '-m') ?? 'auto'),
...
function extractGlobalOptions(input) {
    const args = [];
    let debug = false;
...
async function runAccounts(args) {
...
async function runWakeup(args) {
    const subcommand = (firstPositional(args) ?? 'status');
