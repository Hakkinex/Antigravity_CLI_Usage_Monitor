 ...
 */
export async function executeTrigger(options) {
    const { models, accountEmail, triggerType, triggerSource, customPrompt, maxOutputTokens } = options;
    ...
    for (let i = 0; i < models.length; i += MAX_CONCURRENT_REQUESTS) {
        ...
        const batchResults = await Promise.all(batch.map(modelId => triggerSingleModel(client, modelId, userPrompt, maxOutputTokens)));
 ...
 */
export async function testTrigger(modelId, accountEmail, prompt) {
