...
const CRON_COMMENT_MARKER = 'agy-monitor-wakeup';
/**
...
/**
 * Remove all agy-monitor-wakeup entries from crontab lines
 */
 ...
 */
export function isCronSupported() {
 ...
 */
export async function installCronJob(cronExpression) {
    if (!isCronSupported()) {
        return {
            success: false,
    ...
    try {
        ...
        // Remove any existing agy-monitor entries (both PATH and job lines)
        ...
        // Using 'agy-monitor' instead of absolute paths makes it work anywhere
        const cronLine = `${cronExpression} agy-monitor wakeup trigger --scheduled # ${CRON_COMMENT_MARKER}`;
        // Add new entry
 ...
 */
export async function uninstallCronJob() {
    if (!isCronSupported()) {
 ...
 */
export async function isCronJobInstalled() {
    if (!isCronSupported()) {
        return false;
    }
    try {
...
2. Add these lines:
   PATH=${pathValue}
   ${cronExpression} agy-monitor wakeup trigger --scheduled # ${CRON_COMMENT_MARKER}
...
//# sourceMappingURL=cron-installer.js.map