...
import { loadWakeupConfig, saveWakeupConfig, getOrCreateConfig, getRecentHistory, getLastTrigger } from '../wakeup/index.js';
import { installCronJob, uninstallCronJob, getCronStatus, isCronSupported } from '../wakeup/cron-installer.js';
import { configToCronExpression, getScheduleDescription, getNextRunEstimate } from '../wakeup/schedule-converter.js';
import { executeTrigger, testTrigger } from '../wakeup/trigger-service.js';
import { resolveAccounts, getAccountResolutionStatus } from '../wakeup/account-resolver.js';
import { getAccountManager } from '../accounts/manager.js';
import { debug } from '../core/logger.js';
/**
 * Main wakeup command handler
 */
export async function wakeupCommand(subcommand, args, options) {
...
async function configureWakeup() {
    ...
    if (!config.wakeOnReset && isCronSupported()) {
        ...
        if (installNow) {
            ...
            console.log('   agy-monitor wakeup install');
        }
    }
    console.log('');
}
...
async function installSchedule() {
    ...
    if (!config) {
        ...
        console.log('   Run: agy-monitor wakeup config');
        return;
    }
    if (!config.enabled) {
        console.log('❌ Wake-up is disabled. Enable it first:');
        console.log('   agy-monitor wakeup config');
        return;
    }
    if (config.wakeOnReset) {
    ...
    try {
        ...
        if (result.success) {
            ...
            console.log('');
            console.log('   To check status: agy-monitor wakeup status');
            console.log('   To uninstall: agy-monitor wakeup uninstall');
        }
        else {
...
async function showStatus() {
    ...
    if (!config) {
        ...
        console.log('');
        console.log('   To configure: agy-monitor wakeup config');
        console.log('');
        return;
    }
    // Basic status
    ...
    if (!config.wakeOnReset && config.enabled) {
        ...
        if (cronStatus.installed) {
            ...
            console.log('         Run: agy-monitor wakeup install');
        }
    }
    // Last trigger
