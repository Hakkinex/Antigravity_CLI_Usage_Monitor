...
function showSingleAccountStatus(email) {
    const tokenManager = email
        ...
        : getTokenManager();
    console.log();
    console.log('📍 agy-monitor Status');
    console.log('─'.repeat(40));
    if (!tokenManager.isLoggedIn()) {
        warn('Not logged in');
        console.log();
        info('Run `agy-monitor login` to authenticate.');
        console.log();
        return;
    }
...
function showAllAccountsStatus() {
    ...
    console.log();
    console.log('📍 agy-monitor Status - All Accounts');
    console.log('═'.repeat(60));
    if (emails.length === 0) {
        warn('No accounts found.');
        console.log();
        info('Run `agy-monitor login` to add an account.');
        console.log();
        return;
    }
