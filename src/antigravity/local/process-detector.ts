...
async function detectOnWindows(): Promise<AntigravityProcessInfo | null> {
  try {
    ...
    const { stdout } = await execFileAsync(
      'wmic',
      ['process', 'where', "name like '%antigravity%' or commandline like '%antigravity%'", 'get', 'processid,commandline', '/format:csv'],
...
async function detectOnWindowsPowerShell(): Promise<AntigravityProcessInfo | null> {
  try {
    const { stdout } = await execFileAsync(
      'powershell',
      ['-NoProfile', '-Command', "Get-Process | Where-Object { $_.ProcessName -like '*antigravity*' } | Select-Object Id, ProcessName | ConvertTo-Json"],
...
function redactCommandLine(commandLine: string): string {
  return commandLine
