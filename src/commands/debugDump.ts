...
type Analysis = {
  ...
  methodsSeen: string[];
  sourcesSeen: string[];
  stdoutJsonParseable: boolean;
  stdoutJsonPaths: string[];
  stderrJsonBlocks: Array<{
    label: string;
    parseable: boolean;
    paths: string[];
  }>;
...
export async function runDebugDump(argv: string[]): Promise<void> {
...
function parseDebugDumpOptions(argv: string[]): DebugDumpOptions {
...
function collectDistinctValues(value: unknown, key: string): string[] {
  const values = new Set<string>();
  collectValuesRecursive(value, key, values, 0);
  return [...values].sort();
}

function collectValuesRecursive(value: unknown, key: string, values: Set<string>, depth: number): void {
  if (depth > 8 || value === null || value === undefined) return;
  if (Array.isArray(value)) {
    for (const item of value) collectValuesRecursive(item, key, values, depth + 1);
    return;
  }
...
function hasCachedStatus(parsed: { ok: true; value: unknown } | { ok: false }): boolean {
...
function extractDebugJsonBlocks(stderr: string): Array<{ label: string; json: string }> {
  const blocks: Array<{ label: string; json: string }> = [];
