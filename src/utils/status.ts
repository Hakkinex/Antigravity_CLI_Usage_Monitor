...
export function getModelStatus(percent: number | null, config: MonitorConfig): ModelStatus {
  if (percent === null || Number.isNaN(percent)) return 'unknown';
  if (percent <= 0) return 'exhausted';
  if (percent < config.thresholds.orange) return 'critical';
  if (percent < config.thresholds.yellow) return 'low';
  if (percent < config.thresholds.green) return 'medium';
  return 'healthy';
}
